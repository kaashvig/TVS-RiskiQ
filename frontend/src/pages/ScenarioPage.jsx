...
const MetricCard = ({ label, baseValue, scenarioValue, delta, format, isPositiveGood = true }) => {
  ...
  return (
    <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
      <div className="text-[10px] text-white/40 font-semibold uppercase tracking-wider mb-3">{label}</div>
      <div className="flex items-end justify-between">
        <div>
          <div className="text-[10px] text-white/30 mb-1">Base</div>
          <div className="text-lg font-bold text-white/60">{format(baseValue)}</div>
        </div>
        <motion.div
        ...
        <div className="text-right">
          <div className="text-[10px] text-white/30 mb-1">Stressed</div>
          <motion.div
            initial={{ y: 10, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ type: 'spring', stiffness: 200, delay: 0.3 }}
            className={`text-lg font-bold ${improved ? 'text-emerald-400' : 'text-tvs-red'}`}
          >
      ...
      {delta !== undefined && (
        ...
        >
          <div className={`flex items-center gap-1 text-xs font-semibold ${improved ? 'text-emerald-400' : 'text-tvs-red'}`}>
  ...
  return (
    <div className="p-6 space-y-6 bg-dark-950 min-h-full">
      ...
      >
        ...
        <p className="text-sm text-white/40 mt-1">Stress-test portfolio against market shocks & macro events</p>
      </motion.div>
        ...
        {scenarios.map((s, i) => (
          <motion.button
            ...
            className={`relative glass-card p-5 text-left transition-all ${
              activeScenario === s.key ? '!border-tvs-red/40 !bg-tvs-red/[0.06]' : ''
            }`}
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <div className="text-[10px] text-white/35">{scenarioData[s.key].description.slice(0, 50)}...</div>
              </div>
            </div>
        ...
        >
          ...
          <div className="glass-card p-5">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-base font-bold text-white mb-1">{scenario.label}</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            >
              <h4 className="text-sm font-bold text-white mb-1">Impact Radar</h4>
              <p className="text-xs text-white/35 mb-4">Multi-dimensional stress impact visualization</p>
              ...
              <div className="flex items-center justify-center gap-6 mt-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-white/40">Base Case</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-0.5 rounded" style={{ background: scenario.color }} />
                  <span className="text-[10px] text-white/40">Stressed</span>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="glass-card p-5"
            >
              <h4 className="text-sm font-bold text-white mb-1">Side-by-Side Comparison</h4>
              <p className="text-xs text-white/35 mb-4">Base case vs stressed scenario metrics</p>
                  ...
                  return (
                    <div key={i}>
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-xs text-white/50 font-medium">{item.metric}</span>
                        <span className={`text-xs font-semibold ${isWorse ? 'text-tvs-red' : 'text-emerald-400'}`}>
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] text-white/25 w-10">Base</span>
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] text-white/25 w-10">Stress</span>
                          <div className="flex-1 h-2 rounded-full bg-white/5 overflow-hidden">
                            <motion.div
                              initial={{ width: 0 }}
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-sm font-bold text-white">Stress Test Summary</h4>
            </div>
            <p className="text-sm text-white/60 leading-relaxed">
              Under the <strong className="text-white">{scenario.label}</strong> scenario, the portfolio would experience
